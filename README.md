...
3. Настройте GitHub токен:
  <div style="background-color: #FFFFE0; padding: 5px 5px 0 5px;">
  <p> <b>Важно!</b> Без GitHub токена работа с API будет ограничена 60 запросами/час. Для анализа пользователей с
  большим количеством репозиториев токен обязателен.</p>
  </div>
  
  >[!IMPORTANT]
  >
  >Без GitHub токена работа с API будет ограничена 60 запросами/час. Для анализа пользователей с
  большим количеством репозиториев токен обязателен.
